...
- **旧版误恢复**：早期版本会从历史 transcript / rollout 恢复已结束任务，重启后出现黄灯

当前版本规则：

- **Cursor**：仅由 Hook 实时推送，不再从 transcript 文件恢复
- **Codex**：仅恢复 10 分钟内仍在写入的 Working 会话，不会把旧任务恢复成黄灯

若仍有残留，点击**亮起的黄/红灯**确认即可清除。

### Q: SSH 能用密码连接吗？

**不能。** 远程 Codex 监控需要后台持续轮询，SSH 使用非交互模式（`BatchMode`），无法弹出密码框。
...
然后在 Deva Light 中填写相同私钥路径（支持 `~/.ssh/id_ed25519`），点击「测试」验证。
